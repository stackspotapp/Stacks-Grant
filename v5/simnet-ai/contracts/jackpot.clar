;; title: jackpot
;; version: 1.0.0
;; summary: Stackspots Jackpot pot (simnet / PoX-5)
...
(define-read-only (get-pot-details)
  (ok {
    ...
    pot-reward-amount: (unwrap! (contract-call? .sbtc-token get-balance current-contract) ERR_NOT_FOUND),
...
(define-public (start-stackspot-jackpot (pot-contract <stackspot-pots-trait>))
  (begin
    ...
    ;; Stake treasury into PoX-5 via Fastpool
...
(define-public (claim-pot-reward (pot-contract <stackspot-pots-trait>) (sponsors (list 5 <stackspot-sponsor-trait>)))
  (begin
    ...
    (let (
        ...
        (pot-yield (unwrap! (contract-call? .sbtc-token get-balance current-contract) (err u997)))
        ;; Get stacked reward
