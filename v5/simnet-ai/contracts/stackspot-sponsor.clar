...
(impl-trait .stackspot-sponsor-trait.stackspot-sponsor-trait)
...
(define-constant signer .fastpool)
  ...
  (let
    (
      ...
      (let (
        ...
        (print payload)
        (try! (contract-call? .stackspots log-sponsor-platform payload))
      )
...
(define-public (claim-sponsor-reward (pot-contract <stackspot-pots-trait>) (ticket-id uint))
  (let 
    (
      (sbtc-yield (unwrap! (contract-call? .sbtc-token get-balance current-contract) err_not_found))
    ...
    (if (is-eq (default-to sponsorer (nft-get-owner? pot-ticket ticket-id)) contract-caller)
      (begin 
        ...
        (if (> reward-amount u0) 
          (try! (as-contract? ((with-ft .sbtc-token "sbtc-token" reward-amount)) (try! (contract-call? .sbtc-token transfer reward-amount current-contract (contract-of pot-contract) memo))))
          false
        ) 
        true
      )
...
(define-public (sponsor-event (pot-contract <stackspot-pots-trait>)) 
  (begin
    ...
    (asserts! (contract-call? .stackspots validate-platform-sponsor-contract contract-caller) err_not-authorized)
    (let
      (
...
(define-private (get-total-validated-rule-score (pot-contract <stackspot-pots-trait>)) 
  (let 
    (
      (generated-list (default-to (list ) (contract-call? .stackspot-vrf generate-list u0 rule-sets-count)))
...
(define-read-only (get-minimum-sponsor-amount) 
    (contract-call? .stackspots get-minimum-sponsor-amount)
)
...
(define-public (mint (recipient principal))
	(begin
		(asserts! (contract-call? .stackspots is-contract-allowed-hash recipient) err_not-authorized)
