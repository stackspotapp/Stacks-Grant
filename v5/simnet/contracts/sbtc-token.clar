...
(define-public (protocol-lock
    (amount uint)
    (owner principal)
    (contract-flag (buff 1))
  )
  (begin
    (try! (contract-call? .sbtc-registry is-protocol-caller contract-flag
      contract-caller
    ))
...
(define-public (protocol-burn-locked
    (amount uint)
    (owner principal)
    (contract-flag (buff 1))
  )
...
(define-public (protocol-set-name
...
(define-public (protocol-set-symbol
...
(define-public (protocol-set-token-uri
...
;; --- Public functions
(define-public (transfer-many (recipients (list 200
  {
    amount: uint,
    sender: principal,
    to: principal,
...
(define-private (transfer-many-iter
  ...
  (match result
    index (begin
      (unwrap!
        (transfer (get amount individual-transfer)
          (get sender individual-transfer) (get to individual-transfer)
