...
(define-read-only (validate-pot-value-target-is-met) 
    (>= (var-get total-pot-value) (* (var-get pot-min-amount) (var-get pot-max-participants)))
)
    ...
    (ok
        {
            ...
            pot-reward-amount: (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.sbtc-token get-balance current-contract) ERR_NOT_FOUND),
...
(define-private (delegate-to-pot (amount uint) (participant principal))
    (let
        ...
        (asserts! (is-ok (stx-transfer-memo? amount participant current-contract JOIN_POT_MEMO)) ERR_POT_JOIN_FAILED)
...
(define-public (cancel-pot (pot-contract <stackspot-trait>))
    (begin
      ...
      (try! (as-contract? () (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackspots dispatch-principals pot-contract))))
...
(define-public (start-stackspot-jackpot (pot-contract <stackspot-trait>))
    (begin
        ...
        (try! (as-contract? () (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackspots delegate-treasury pot-contract current-contract))))
...
(define-public (claim-pot-reward (pot-contract <stackspot-trait>))
    (let
        (
            ...
            (pot-yield (unwrap! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.sbtc-token get-balance current-contract) (err u997))) ;; Get stacked reward
        ...
        ;; Disburse rewards
        (try! (as-contract? () (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackspots dispatch-rewards pot-contract))))
...
(define-read-only (get-pot-cycle) (ok (var-get pot-cycle)))
(define-read-only (get-pot-min-amount) (ok (var-get pot-min-amount)))
(define-read-only (get-pot-max-participants) (ok (var-get pot-max-participants)))
(define-read-only (get-pot-name) (ok (var-get pot-name)))
(define-read-only (get-pot-type) (ok (var-get pot-type)))
(define-read-only (get-pot-origin-contract-sha-hash) (contract-hash? current-contract))
(define-read-only (get-pot-reward-token) (ok "sbtc"))

(as-contract? () (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.sim-pox-4-multi-pool-v1 allow-contract-caller 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackspot-distribute none)))
(as-contract? () (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.sim-pox-4 allow-contract-caller 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.sim-pox4-multi-pool-v1 none)))
...
(define-read-only (invariant-no-joins-after-lock)
    (let (
            (joins (default-to u0 (get called (map-get? context "join-pot"))))
        )
        (if (var-get locked)
