...
(define-map active-protocol-roles
  principal
  (buff 1)
)
...
(define-map completed-withdrawal-sweep
  uint
  {
    sweep-txid: (buff 32),
...
(define-read-only (get-withdrawal-request (id uint))
  (match (map-get? withdrawal-requests id)
    request (some (merge request { status: (map-get? withdrawal-status id) }))
    none
  )
)
...
(define-read-only (get-completed-deposit
    (txid (buff 32))
    (vout-index uint)
  )
  (map-get? completed-deposits {
    txid: txid,
...
;; This function returns the fields of the completed-deposits map.
(define-read-only (get-deposit-status
    (txid (buff 32))
    (vout-index uint)
  )
  (map-get? deposit-status {
    txid: txid,
    vout-index: vout-index,
  })
)
...
(define-public (create-withdrawal-request
  ...
  (let ((id (increment-last-withdrawal-request-id)))
...
(define-public (complete-deposit
  ...
  (begin
    ...
    (map-insert deposit-status {
      txid: txid,
      vout-index: vout-index,
    }
      true
    )
    (map-insert completed-deposits {
      txid: txid,
      vout-index: vout-index,
    } {
      amount: amount,
...
(define-public (rotate-keys
  ...
  (begin
    ...
    (asserts! (map-insert aggregate-pubkeys new-aggregate-pubkey true)
...
(define-private (increment-last-withdrawal-request-id)
  (let ((next-value (+ u1 (var-get last-withdrawal-request-id))))
...
(define-read-only (is-protocol-caller
  ...
  (begin
    ...
    (asserts!
      (is-eq (some contract) (map-get? active-protocol-contracts contract-flag))
      ERR_UNAUTHORIZED
    )
    ;; Check that flag matches the contract-caller
    (asserts!
      (is-eq (some contract-flag) (map-get? active-protocol-roles contract))
      ERR_UNAUTHORIZED
    )
    (ok true)
  )
