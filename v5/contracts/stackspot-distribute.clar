...
(define-public (dispatch-rewards (contract <stackspot-trait>))
  (let (
      ...
      (pot-treasury (unwrap! (contract-call? contract get-pot-treasury) ERR_NOT_FOUND))
      (pot-yield (unwrap! (contract-call? .sbtc-token get-balance pot-treasury)
        ERR_NOT_FOUND
      ))
      (pot-id (unwrap! (contract-call? contract get-pot-id) ERR_NOT_FOUND))
      ;; Pot Fee
    ...
    (and
      ...
      (try! (dispatch-rewards-with-sbtc platform-royalty-reward pot-treasury
        platform-royalty-address
        (to-consensus-buff? "platform royalty reward")
      ))
    )
    ...
    (and
      (> pot-fee u0)
      (try! (dispatch-rewards-with-sbtc pot-fee pot-treasury pot-owner-address
        (to-consensus-buff? "pot fee reward")
      ))
    )
    ...
    (and
      ...
      (try! (dispatch-rewards-with-sbtc pot-starter-reward pot-treasury
        pot-starter-address (to-consensus-buff? "pot starter reward")
      ))
    )
    ...
    (and
      (> claimer-reward u0)
      (try! (dispatch-rewards-with-sbtc claimer-reward pot-treasury claimer-address
        (to-consensus-buff? "claimer reward")
      ))
    )
    ...
    (and
      (> winner-reward u0)
      (try! (dispatch-rewards-with-sbtc winner-reward pot-treasury winner-address
        (to-consensus-buff? "winner reward")
      ))
    )

    (try! (contract-call? .stackspot-winners log-winner
      (unwrap!
        (to-consensus-buff? {
          ;; Pot Values
          ...
          pot-max-participants: (unwrap! (contract-call? contract get-pot-max-participants)
            ERR_NOT_FOUND
          ),
...
(define-public (delegate-treasury
  ...
  (let (
    ...
    (contract-call? .sim-pox-4-multi-pool-v1 delegate-stx amount-ustx
      (unwrap! (to-consensus-buff? { c: "sbtc" }) ERR_NOT_FOUND)
    )
  )
)
