...
(define-public (start-stackspot-jackpot (pot-contract <stackspot-trait>))
    (begin
        ...
        (try! (as-contract? () (try! (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackspots delegate-treasury pot-contract pot-treasury-address))))
...
(define-public (claim-pot-reward (pot-contract <stackspot-trait>))
    (let
        ...
        (asserts! (not (> (var-get next-payment-id) total-participants)) ERR_MAX_PARTICIPANTS_REACHED)
...
(define-data-var pot-cycle uint (var-get last-participant))
