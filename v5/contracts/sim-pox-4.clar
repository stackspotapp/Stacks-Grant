...
(define-read-only (get-stacker-info (stacker principal))
  (match (map-get? stacking-state { stacker: stacker })
    stacking-info
    (if (<=
        (+ (get first-reward-cycle stacking-info) (get lock-period stacking-info))
...
(define-read-only (get-reward-set-size (reward-cycle uint))
  (default-to u0
    (get len
      (map-get? reward-cycle-pox-address-list-len { reward-cycle: reward-cycle })
    ))
)
...
(define-private (append-reward-cycle-pox-addr
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
  ...
  (let ((sz (get-reward-set-size reward-cycle)))
    ...
    (map-set reward-cycle-pox-address-list-len { reward-cycle: reward-cycle } { len: (+ u1 sz) })
    sz
  )
)
...
(define-read-only (get-reward-set-pox-address
    (reward-cycle uint)
    (index uint)
  )
  (map-get? reward-cycle-pox-address-list {
    reward-cycle: reward-cycle,
    index: index,
  })
)
...
(define-private (add-pox-addr-to-ith-reward-cycle
  ...
  (let (
      ...
      (reward-set-index (if (< i num-cycles)
        (let (
            ...
            (reward-index
              ...
              (append-reward-cycle-pox-addr (get pox-addr params) reward-cycle
                (get amount-ustx params) (get stacker params)
                (get signer params)
              ))
          )
          ;; update running total
          (map-set reward-cycle-total-stacked { reward-cycle: reward-cycle } { total-ustx: (+ (get amount-ustx params) total-ustx) })
          (some reward-index)
        )
        none
      ))
      (next-i (if (< i num-cycles)
        (+ i u1)
        i
      ))
    )
    {
      ...
      reward-set-indexes: (match reward-set-index
        new (unwrap-panic (as-max-len? (append (get reward-set-indexes params) new) u12))
...
;; No checking is done.
(define-private (add-pox-addr-to-reward-cycles
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
  ...
  (let (
    ...
    (asserts! (is-eq num-cycles (len reward-set-indexes))
...
(define-private (add-pox-partial-stacked-to-ith-cycle
  ...
  (let (
    ...
    (let ((current-amount (default-to u0
        (get stacked-amount
          (map-get? partial-stacked-by-cycle {
            sender: tx-sender,
            pox-addr: pox-addr,
            reward-cycle: reward-cycle,
          })
...
;; A PoX address can be added to at most 12 consecutive cycles.
;; No checking is done.
(define-private (add-pox-partial-stacked
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
  ...
  (let ((cycle-indexes (list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11)))
    (fold add-pox-partial-stacked-to-ith-cycle cycle-indexes {
      pox-addr: pox-addr,
      reward-cycle: first-reward-cycle,
      num-cycles: num-cycles,
      amount-ustx: amount-ustx,
    })
    true
...
(define-read-only (check-pox-lock-period (lock-period uint))
  (and
    (>= lock-period MIN_POX_REWARD_CYCLES)
...
(define-read-only (can-stack-stx
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
...
(define-read-only (minimal-can-stack-stx
  ...
  (begin
    ...
    (asserts! (> amount-ustx u0) (err ERR_STACKING_INVALID_AMOUNT))
    ...
    (asserts!
      (check-pox-addr-hashbytes (get version pox-addr) (get hashbytes pox-addr))
      (err ERR_STACKING_INVALID_POX_ADDRESS)
    )
...
(define-public (stack-stx
  ...
  (let (
    ...
    (try! (consume-signer-key-authorization pox-addr (- first-reward-cycle u1)
      "stack-stx" lock-period signer-sig signer-key amount-ustx max-amount
      auth-id
    ))
    ...
    (let ((reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-reward-cycle lock-period
        amount-ustx tx-sender signer-key
      ))))
      ;; add stacker record
      (map-set stacking-state { stacker: tx-sender } {
        pox-addr: pox-addr,
...
(define-public (revoke-delegate-stx)
  (let ((last-delegation-state (get-check-delegation tx-sender)))
    ;; must be called directly by the tx-sender or by an allowed contract-caller
    (asserts! (check-caller-allowed) (err ERR_STACKING_PERMISSION_DENIED))
    (asserts! (is-some last-delegation-state)
      (err ERR_DELEGATION_ALREADY_REVOKED)
    )
    (asserts! (map-delete delegation-state { stacker: tx-sender })
      (err ERR_DELEGATION_ALREADY_REVOKED)
    )
...
(define-public (delegate-stx
  ...
  (begin
    ...
    (match pox-addr
      address (asserts! (check-pox-addr-version (get version address))
        (err ERR_STACKING_INVALID_POX_ADDRESS)
      )
      true
    )
    ...
    (match pox-addr
      pox-tuple (asserts!
        (check-pox-addr-hashbytes (get version pox-tuple)
          (get hashbytes pox-tuple)
        )
        (err ERR_STACKING_INVALID_POX_ADDRESS)
      )
      true
    ...
    (map-set delegation-state { stacker: tx-sender } {
      amount-ustx: amount-ustx,
...
(define-read-only (verify-signer-key-sig
  ...
  (begin
    ...
    (match signer-sig-opt
      ...
      (ok (asserts!
        (is-eq
          (unwrap!
            ...
            (err ERR_INVALID_SIGNATURE_RECOVER)
          )
          signer-key
        )
      ...
      (ok (asserts!
        (default-to false
          (map-get? signer-key-authorizations {
            signer-key: signer-key,
            reward-cycle: reward-cycle,
            period: period,
            topic: topic,
...
(define-private (consume-signer-key-authorization
  ...
  (begin
    ...
    (try! (verify-signer-key-sig pox-addr reward-cycle topic period signer-sig-opt
...
;;
;; *New in Stacks 2.1.*
(define-private (inner-stack-aggregation-commit
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
  ...
  (let ((partial-stacked
    ...
    (let ((amount-ustx (get stacked-amount partial-stacked)))
      (try! (consume-signer-key-authorization pox-addr reward-cycle "agg-commit" u1
        signer-sig signer-key amount-ustx max-amount auth-id
      ))
      ...
      (let (
          (add-pox-addr-info (add-pox-addr-to-ith-reward-cycle u0 {
            pox-addr: pox-addr,
          ...
          (pox-addr-index (unwrap-panic (element-at (get reward-set-indexes add-pox-addr-info) u0)))
        )
        ...
        (map-delete partial-stacked-by-cycle {
          pox-addr: pox-addr,
          sender: tx-sender,
          reward-cycle: reward-cycle,
        })
        (map-set logged-partial-stacked-by-cycle {
          pox-addr: pox-addr,
          sender: tx-sender,
          reward-cycle: reward-cycle,
        }
...
(define-public (stack-aggregation-increase
  ...
  (let ((partial-stacked
    ...
    (let (
        (partial-amount-ustx (get stacked-amount partial-stacked))
      ...
      ;; Validate that amount is less than or equal to `max-amount`
      (asserts! (>= max-amount increased-entry-total)
        (err ERR_SIGNER_AUTH_AMOUNT_TOO_HIGH)
      )

      ;; Validate that signer-key matches the existing signer-key
      (asserts! (is-eq existing-signer-key signer-key)
        (err ERR_INVALID_SIGNER_KEY)
      )
      ...
      (try! (consume-signer-key-authorization pox-addr reward-cycle "agg-increase" u1
        signer-sig signer-key increased-entry-total max-amount auth-id
      ))
      ...
      (map-set reward-cycle-total-stacked { reward-cycle: reward-cycle } { total-ustx: increased-cycle-total })
...
(define-public (delegate-stack-stx
  ...
  (let (
    ...
    (let ((delegation-info (unwrap! (get-check-delegation stacker)
        (err ERR_STACKING_PERMISSION_DENIED)
      )))
      ...
      (asserts!
        (match (get until-burn-ht delegation-info)
          until-burn-ht (>= until-burn-ht unlock-burn-height)
          true
        )
    ...
    ;; add stacker record
    (map-set stacking-state { stacker: stacker } {
      pox-addr: pox-addr,
      first-reward-cycle: first-reward-cycle,
      reward-set-indexes: (list),
      lock-period: lock-period,
  ...
  (let (
    ...
    (if (> first-cycle reward-cycle)
      ...
      (some {
        ...
        signer-key: (get signer-key data),
      })
      (let (
          (existing-entry (unwrap-panic (map-get? reward-cycle-pox-address-list {
            reward-cycle: reward-cycle,
            index: reward-cycle-index,
          })))
        ...
        ;; stacker must match
        (asserts! (is-eq (get stacker existing-entry) (some (get stacker data)))
          none
        )
        ...
        ;; update the total
        (map-set reward-cycle-total-stacked { reward-cycle: reward-cycle } { total-ustx: total-ustx })
        (some {
          first-cycle: first-cycle,
          reward-cycle: (+ u1 reward-cycle),
          stacker: (get stacker data),
...
(define-public (stack-increase
  ...
  (let (
      ...
      (stacker-state (unwrap! (map-get? stacking-state { stacker: tx-sender })
    ...
    (asserts! (> amount-stacked u0) (err ERR_STACK_INCREASE_NOT_LOCKED))
    ;; must be called with positive `increase-by`
    (asserts! (>= increase-by u1) (err ERR_STACKING_INVALID_AMOUNT))
    ...
    ;; Validate that amount is less than or equal to `max-amount`
    (asserts! (>= max-amount (+ increase-by amount-stacked))
      (err ERR_SIGNER_AUTH_AMOUNT_TOO_HIGH)
    )

    ;; Verify signature from delegate that allows this sender for this cycle
    (try! (consume-signer-key-authorization cur-pox-addr cur-cycle "stack-increase"
      cur-period signer-sig signer-key increase-by max-amount auth-id
    ))

    ;; update reward cycle amounts
    (asserts!
      (is-some (fold increase-reward-cycle-entry (get reward-set-indexes stacker-state)
        (some {
          first-cycle: first-increased-cycle,
    ...
    (ok {
      stacker: tx-sender,
      total-locked: (+ amount-stacked increase-by),
    })
  )
)
...
(define-public (stack-extend
  ...
  (let (
      ...
      (first-reward-cycle (if (> cur-cycle cur-first-reward-cycle)
        cur-cycle
        cur-first-reward-cycle
      ))
    )
    ...
    ;; Verify signature from delegate that allows this sender for this cycle
    (try! (consume-signer-key-authorization pox-addr cur-cycle "stack-extend"
      extend-count signer-sig signer-key u0 max-amount auth-id
    ))
    ...
    (let (
      ...
      (asserts! (> first-extend-cycle cur-cycle)
        (err ERR_STACKING_INVALID_LOCK_PERIOD)
      )
      ...
      (asserts! (> amount-ustx u0) (err ERR_STACK_EXTEND_NOT_LOCKED))
      ...
      ;;   for the new cycles
      (let (
          (extended-reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-extend-cycle extend-count
            amount-ustx tx-sender signer-key
          )))
          (reward-set-indexes
            ;; use the active stacker state and extend the existing reward-set-indexes
            (let (
                (cur-cycle-index (- first-reward-cycle (get first-reward-cycle stacker-state)))
                ...
                (new-list (concat
                  (default-to (list)
                    (slice? old-indexes cur-cycle-index (len old-indexes))
                  )
...
(define-public (delegate-stack-increase
  ...
  (let (
    ...
    (let (
        ...
        (stacker-state (unwrap! (map-get? stacking-state { stacker: stacker })
          (err ERR_STACK_INCREASE_NOT_LOCKED)
        ))
      )
      ;; must be called directly by the tx-sender or by an allowed contract-caller
      ...
      (asserts!
        (is-eq
          (unwrap! (get delegated-to stacker-state)
            (err ERR_STACKING_NOT_DELEGATED)
          )
          tx-sender
        )
      ...
      (asserts! (> existing-lock u0) (err ERR_STACK_INCREASE_NOT_LOCKED))
      ...
      ;; stacker must have delegated to the caller
      (let (
          (delegation-info (unwrap! (get-check-delegation stacker)
            (err ERR_STACKING_PERMISSION_DENIED)
          ))
        ...
        ;; delegation must not expire before lock period
        (asserts!
          (match delegated-until
            until-burn-ht (>= until-burn-ht unlock-height)
            true
          )
          (err ERR_DELEGATION_EXPIRES_DURING_LOCK)
        )
      ...
      (try! (minimal-can-stack-stx pox-addr new-total-locked first-increase-cycle
        (+ u1 (- last-increase-cycle first-increase-cycle))
      ))

      ;; register the PoX address with the amount stacked via partial stacking
      ;;   before it can be included in the reward set, this must be committed!
      (add-pox-partial-stacked pox-addr first-increase-cycle cycle-count
        increase-by
      )
  ...
  (let (
    ...
    (let (
      ...
      (let ((delegation-info (unwrap! (get-check-delegation stacker)
        ...
        (asserts!
          (match (get until-burn-ht delegation-info)
            until-burn-ht (>= until-burn-ht new-unlock-ht)
            true
          )
          (err ERR_DELEGATION_EXPIRES_DURING_LOCK)
        )
      ...
      ;; register the PoX address with the amount stacked via partial stacking
      ;;   before it can be included in the reward set, this must be committed!
      (add-pox-partial-stacked pox-addr first-extend-cycle extend-count
        amount-ustx
      )
...
;;
;; *New in Stacks 3.0*
(define-public (set-signer-key-authorization
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
  ...
  (begin
    ;; must be called directly by the tx-sender or by an allowed contract-caller
    (asserts! (check-caller-allowed) (err ERR_NOT_ALLOWED))
    ...
    (asserts! (>= reward-cycle (current-pox-reward-cycle))
      (err ERR_INVALID_REWARD_CYCLE)
    )
    (map-set signer-key-authorizations {
      pox-addr: pox-addr,
      period: period,
      reward-cycle: reward-cycle,
      topic: topic,
...
;; *New in Stacks 2.1*
(define-read-only (get-partial-stacked-by-cycle
    (pox-addr {
      version: (buff 1),
      hashbytes: (buff 32),
    })
