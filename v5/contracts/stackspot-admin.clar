...
(define-public (add-pot-contract-hash (contract <stackspot-trait>) (state bool))
    (let 
        (
            (hash (unwrap! (contract-hash? (contract-of contract)) ERR_NOT_FOUND))
        ) 
...
(define-read-only (is-contract-allowed-hash (contract-address principal)) 
    (map-get? allowed-contract-hash (unwrap! (contract-hash? contract-address) none))
)
;; Initialize admins
