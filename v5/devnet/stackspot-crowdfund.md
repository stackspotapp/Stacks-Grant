...
| `ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5` | Primary contract deployer and owner of the `stackspot-crowdfund` contract used throughout the test scenarios. |
...
| `ST2JHG361ZXG51QTKY2NQCVBPPRRE2KZB1HR05NNC` | Participant Account #1 used for joining crowdfund pots and testing contribution flows. |
...
| `ST3AM1A56AK2C1XAFJ4115ZSV26EB49BVQ10MGCS0` | Crowdfund operator/executor account responsible for starting crowdfund execution and handling cancellation operations. |
...
| `ST3PF13W7Z0RRM42A8VZRVFQ75SV1K26RXEP8YGKJ` | Additional reserve testing account available for extended crowdfund or participant simulations. |
...
# SCENARIO 1:
This scenario demonstrates the complete happy-path execution flow of the StackSpot crowdfunding crowdfund lifecycle.  
The flow is executed sequentially to simulate a real-world successful operation — beginning from protocol/admin setup,  
pot initialization, participant deposits, crowdfund activation, reward funding, and finally reward claiming.
...
- Crowdfund execution starts correctly after setup completion.
...
;; Sets the approved crowdfund/pot contract hash inside the admin contract.
...
(contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.stackspot-admin set-pot-contract-hash 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund true)
...
;; Initializes a new crowdfund/pot instance.
;;
;; Parameters:
;; u1                     -> Pot ID
...
;; required for subsequent participation and crowdfund operations.
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund init-pot u1 u100000 u100 "test-001" "stackspot-crowdfund" 'STNHKEPYEPJ8ET55ZZ0M5A34J0R3N5FM2CMMMAZ6)
```
...
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund get-pot-details)
...
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund join-pot u100000000)
...
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund join-pot u200000000)
...
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund join-pot u300000000)
...
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund get-pot-participants)
```
...
;; Starts the StackSpot crowdfund execution process for the specified pot.
;;
;; This step simulates:
;; - Delegation finalization
;; - Crowdfund activation
;; - Stacking lifecycle initiation
;; - Reward generation eligibility
;;
;; At this stage, the pot should transition into an active crowdfund state.
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund start-stackspot-crowdfund 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund)
```
...
;; Mints and transfers sBTC rewards into the crowdfund contract.
;;
;; Parameters:
;; u2000                  -> Reward amount being minted
;; contract principal     -> Receiving crowdfund contract
;; 0x01                   -> Optional memo/version payload
;;
;; This simulates external reward funding after successful stacking operations.
(contract-call? .sbtc-token protocol-mint u2000 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund 0x01)
```
...
;; Executes crowdfund reward claiming flow.
;;
;; Expected successful operations:
;; - Winner validation
;; - Reward eligibility verification
;; - Transfer of accumulated crowdfund rewards
;; - Internal state updates marking the reward as claimed
;;
;; This represents the final stage of the complete crowdfund lifecycle.
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund claim-pot-reward 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund)
...
# SCENARIO 2:
This flow tests the pot cancellation functionality after participant contributions have been made, but no delegation or crowdfund execution occurs within a complete stacking cycle.  

The scenario simulates a failed or inactive crowdfund lifecycle where:
- Participants successfully contribute funds into the pot.
- No delegation or crowdfund activation is executed.
...
;; Registers and authorizes the crowdfund/pot contract hash
...
;; Creates and initializes a new crowdfunding crowdfund pot.
;;
;; Parameters:
;; u1                    -> Pot ID
;; u100000               -> Minimum contribution amount
...
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund init-pot u1 u100000 u100 "test-001" "stackspot-crowdfund")
```
...
;; Advances the simulated Bitcoin burn chain by 2400 blocks.
;;
;; Unlike Scenario 1, no delegation or crowdfund activation
;; occurs before the cycle advancement.
;;
;; This simulates an inactive or abandoned crowdfund lifecycle
...
;; Cancels the inactive crowdfund pot after the required
;; cycle expiration period has passed.
;;
;; Expected contract operations:
;; - Verify cancellation eligibility
;; - Ensure no active delegation/crowdfund exists
...
;; for inactive or abandoned crowdfund executions.
(contract-call? 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund cancel-pot 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5.stackspot-crowdfund)
```
