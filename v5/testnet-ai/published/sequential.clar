;; title: sequential
;; version: 1.0.0
;; summary: Stackspots Sequential pot (testnet / PoX-5)
;; description: Round-robin payouts via next-payment-id. Staking + payouts happen on this contract.

;; --- Traits
(impl-trait 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspot-pots-trait.stackspot-pots-trait)
(use-trait stackspot-pots-trait 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspot-pots-trait.stackspot-pots-trait)
(use-trait stackspot-sponsor-trait 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspot-sponsor-trait.stackspot-sponsor-trait)
...
;; Widen a consensus-serialized event to `'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots` `(buff 2048)` logs.
...
(define-constant PLATFORM_ADDRESS (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots get-platform-treasury))
...
(define-private (extend-stake)
  (begin
    (try! (contract-call? 'ST000000000000000000002AMW42H.pox-5 stake-update 'ST31XHNM0GZ2K978FPP4QA3STNQ73Z8C9G9MJEPK2.signer-manager 'ST31XHNM0GZ2K978FPP4QA3STNQ73Z8C9G9MJEPK2.signer-manager u1 u0 none))
...
(define-private (revoke-stake)
  (begin
    (try! (contract-call? 'ST000000000000000000002AMW42H.pox-5 unstake 'ST31XHNM0GZ2K978FPP4QA3STNQ73Z8C9G9MJEPK2.signer-manager))
...
(define-private (pull-remaining-staking-rewards)
  (let (
      ...
      (offsets (unwrap! (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspot-vrf generate-list u0 count) ERR_NOT_FOUND))
...
(define-read-only (get-pot-participants)
  (let (
      ...
      (n (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspot-vrf generate-list u0 participants-count)) 
...
(define-read-only (get-sponsors-addresses)
  (let 
    (
      (n (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspot-vrf generate-list u0 (var-get last-sponsors-count)))
      (participants 
        (match n
...
(define-read-only (get-pot-id)
  (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots get-token-id pot-treasury-address)
)
...
(define-private (delegate-to-pot (amount uint) (participant principal))
  (let (
    ...
    (let (
      ...
      (print payload)
      (try! (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots log-join-pot payload))
    )
...
(define-public (join-pot-as-sponsor (amount uint) (sponsor principal))
  (begin
    ...
    (let (
      ...
      (print payload)
      (try! (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots log-join-pot-as-sponsor payload))
    )
...
(define-public (cancel-pot (pot-contract <stackspot-pots-trait>))
  (begin
    ...
    (let (
      ...
      (print payload)
      (try! (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots log-cancel-pot payload))
    )
...
(define-public (fall-back-cancel (pot-contract <stackspot-pots-trait>))
  (begin
    ...
    ;; Pull any remaining signer-manager sBTC for the defined stake term
    (try! (pull-remaining-staking-rewards))

    ;; Schedule unlock at the next cycle boundary if still actively staking
    (if (is-some (contract-call? 'ST000000000000000000002AMW42H.pox-5 get-staker-info current-contract))
      (begin
        (try! (as-contract? ()
          (try! (revoke-stake))
        ))
    ...
    (let (
      ...
      (try! (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots log-fall-back-cancel
...
(define-public (start-stackspot-sequential-pot (pot-contract <stackspot-pots-trait>))
  (begin
    ...
    ;; Stake treasury into PoX-5 via signer-manager for one cycle (extend on each later claim)
...
(define-public (claim-pot-reward (pot-contract <stackspot-pots-trait>) (sponsors (list 5 <stackspot-sponsor-trait>)))
  (begin
    ...
    ;; Pull the next signer-manager reward-cycle onto this pot
    ...
    (try! (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots log-claim-pot-reward
      (to-stackspots-buff (unwrap! (as-max-len? (unwrap! (to-consensus-buff? {
        event: "claim-pot-reward",
        pot-participants-count: total-participants,
        pot-value: (var-get total-pot-value),
...
(define-public (init-pot (min-amount uint) (max-participants uint) (name (string-ascii 255)) (contract <stackspot-pots-trait>) (sponsors (list 5 <stackspot-sponsor-trait>)))
  (begin
    ...
    (let (
      ...
      (contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots register-pot payload contract)
    )
  )
)
...
(print PRE_INIT_LOG)
(contract-call? 'ST2JWVB86PMG4MH4XWECB4W4KTKXV1NMCMMEJYQ3Y.stackspots log-pre-init PRE_INIT_LOG)
