...
;; --- Traits
(impl-trait 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspot-pots-trait.stackspot-pots-trait)
(use-trait stackspot-pots-trait 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspot-pots-trait.stackspot-pots-trait)
(use-trait stackspot-sponsor-trait 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspot-sponsor-trait.stackspot-sponsor-trait)
...
;; Widen a consensus-serialized event to `'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots` `(buff 2048)` logs.
...
(define-constant PLATFORM_ADDRESS (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots get-platform-treasury))
...
(define-private (pull-remaining-staking-rewards)
  (let (
      ...
      (offsets (unwrap! (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspot-vrf generate-list u0 count) ERR_NOT_FOUND))
...
(define-read-only (get-pot-participants)
  (let (
      ...
      (n (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspot-vrf generate-list u0 participants-count)) 
...
(define-read-only (get-sponsors-addresses)
  (let 
    (
      (n (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspot-vrf generate-list u0 (var-get last-sponsors-count)))
      (participants 
        (match n
...
(define-read-only (get-pot-id)
  (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots get-token-id pot-treasury-address)
)
...
(define-private (delegate-to-pot (amount uint) (participant principal))
  (let (
    ...
    (let (
      ...
      (print payload)
      (try! (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots log-join-pot payload))
    )
...
(define-public (join-pot-as-sponsor (amount uint) (sponsor principal))
  (begin
    ...
    (let (
      ...
      (print payload)
      (try! (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots log-join-pot-as-sponsor payload))
    )
...
(define-public (cancel-pot (pot-contract <stackspot-pots-trait>))
  (begin
    ...
    (let (
      ...
      (print payload)
      (try! (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots log-cancel-pot payload))
    )
...
(define-public (fall-back-cancel (pot-contract <stackspot-pots-trait>))
  (begin
    ...
    (let (
      ...
      (try! (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots log-fall-back-cancel
...
(define-public (claim-pot-reward (pot-contract <stackspot-pots-trait>) (sponsors (list 5 <stackspot-sponsor-trait>)))
  (begin
    ...
    (try! (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots log-claim-pot-reward
      (to-stackspots-buff (unwrap! (as-max-len? (unwrap! (to-consensus-buff? {
        event: "claim-pot-reward",
        pot-participants-count: total-participants,
        pot-value: (var-get total-pot-value),
...
(define-public (init-pot (min-amount uint) (max-participants uint) (name (string-ascii 255)) (contract <stackspot-pots-trait>) (sponsors (list 5 <stackspot-sponsor-trait>)))
  (begin
    ...
    (let (
      ...
      (contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots register-pot payload contract)
    )
  )
)
...
(print PRE_INIT_LOG)
(contract-call? 'ST300KR5ZKJCGQGSFF84CNAQ3V557CNXWM94PFDG5.stackspots log-pre-init PRE_INIT_LOG)
