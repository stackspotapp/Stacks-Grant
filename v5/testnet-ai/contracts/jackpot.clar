...
(define-private (pull-all-staking-rewards)
  (let (
      (offsets (unwrap! (contract-call? 'ST4B1RB4STWAGXDYH05CAK2T14BBC2CJ6E1BGG7D.stackspot-vrf generate-list u0 (var-get staked-cycles)) ERR_NOT_FOUND))
...
(define-read-only (get-pot-id)
  (contract-call? 'ST4B1RB4STWAGXDYH05CAK2T14BBC2CJ6E1BGG7D.stackspots get-token-id current-contract)
)
...
(define-read-only (get-random-index (participant-count uint))
  (let (
      ...
      (vrf-random-digit (unwrap! (contract-call? 'ST4B1RB4STWAGXDYH05CAK2T14BBC2CJ6E1BGG7D.stackspot-vrf get-random-uint-at-block stacks-block-height) ERR_NOT_FOUND))
    )
