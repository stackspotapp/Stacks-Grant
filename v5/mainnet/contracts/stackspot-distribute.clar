...
(define-read-only (get-pox-info)
  (unwrap-panic (contract-call? 'SP000000000000000000002Q6VF78.pox-4 get-pox-info))
)
...
(define-public (delegate-treasury (contract <stackspot-trait>) (delegate-to principal))
  (let (
    ...
    (contract-call? 'SPMPMA1V6P430M8C91QS1G9XJ95S59JS1TZFZ4Q4.pox4-multi-pool-v1 delegate-stx amount-ustx (unwrap! (to-consensus-buff? { c: "sbtc" }) ERR_NOT_FOUND))
  )
)
...
(define-public (extend-delegate-treasury (contract <stackspot-trait>))
  (let (
    ...
    (try! (contract-call? 'SPMPMA1V6P430M8C91QS1G9XJ95S59JS1TZFZ4Q4.pox4-multi-pool-v1 delegate-stack-stx treasury-address))
